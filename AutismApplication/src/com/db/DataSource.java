 ...
 * using db at any point in time :) Usage Example: Call this from your activity,
 * param 'this' should be a context (i.e. Activity object) DataSource dataSource
 * = new DataSource(this); Then open a writable version of db
 * dataSource.openWritableDB(); Then for example to create a task call
 * dataSource.createTaskData(String name); You can also call deleteData(Data
 * data); where the data param is one of TaskData, ReminderData, NoteData,
 * PhotoData, ContactData this will delete only a single row that corresponds to
 * the passed in data object A data object corresponds to a single row You can
 * also call to get all data from a particular table List<TaskData> list =
 * getAllTaskData(); you now have a list you can go through When you are finally
 * done with the db you can call dataSource.close();
 */
